...
train_dataset =  vxm.nlst.NLST("/home/varsha/data/NLST/", json_file_name,
                                downsampled=False, 
                                masked=False,
                             is_norm=True)
valid_set =  vxm.nlst.NLST("/home/varsha/data/NLST/", json_file_name,
                                downsampled=False, 
...
train_dataloader = DataLoader(overfit_set, batch_size=1, shuffle=True, num_workers=4)
val_dataloader = DataLoader(overfit_set, batch_size=1, shuffle=False, num_workers=4)

print("Number of training samples: ", len(overfit_set))
print("Number of val samples: ", len(overfit_set))
...
if args.load_model:
    ...
    model = vxm.networks.VxmDenseSemisupervised.load(args.load_model, device)
else:
    # otherwise configure new model
    model = vxm.networks.VxmDenseSemisupervised(
        inshape=(224,192,224),
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(val_dataloader):
            ...
            target_fixed = target_fixed[0,0,:,119,:]
            ...
            target_moving = moving_mask[0,:,119,:]
            flow_x = y_pred[1][0,0,:,119,:].to('cpu').detach().numpy()
            flow_y = y_pred[1][0,1,:,119,:].to('cpu').detach().numpy()
